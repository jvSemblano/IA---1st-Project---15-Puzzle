   ...
   return;
        ...
        static void GULOSA( Node jogoinicial, Node jogofinal){ 
            ...
            while(!queue.isEmpty()){
                ...
                filhotes = makeDescendants(temporary,jogofinal);
        ...
        int prof = temporary.profundidade+1;
        if(prof<=limite) {
          filhotes = makeDescendants(temporary);
          for(int i = 0; i < filhotes.size(); i++){
...
//para heuristicas
static Node moveUp(Node game, Node jogofinal){
...
static Node moveDown(Node game, Node jogofinal){
...
static Node moveLeft(Node game, Node jogofinal){
...
static Node moveRight(Node game, Node jogofinal){
...
static LinkedList<Node> makeDescendants(Node pai, Node jogofinal){
   LinkedList<Node> children = new LinkedList<Node>();
   if(pai.getY() != 0){
      children.add(moveUp(pai,jogofinal));
  }

  if(pai.getY() != 3){
      children.add(moveDown(pai, jogofinal));
  }

  if(pai.getX() != 0){
      children.add(moveLeft(pai, jogofinal));
  }

  if(pai.getX() != 3){
      children.add(moveRight(pai, jogofinal));
  }
